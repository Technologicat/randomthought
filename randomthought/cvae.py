...
def make_codec(variant: int, latent_dim: int) -> typing.Tuple[tf.keras.Model, tf.keras.Model]:
    """Set up compatible encoder and decoder neural networks for a CVAE for 2D bitmap image data.

    Mainly meant for use by `CVAE.__init__`.

    `variant`: We provide several variants of the CVAE, with different neural network geometries.
               See the source code for details.

    `latent_dim`: The dimension of the latent space of the CVAE.

                  More dimensions means more capacity to capture more explaining factors from the training data.

                  How high the dimension needs to be depends on the dimension of the data manifold,
                  which is in general unknown.

                  If the dimension is too low for the given training data, the CVAE will, essentially,
                  forcibly embed the manifold into `latent_dim` dimensions nevertheless, crumpling it
                  in the process. Then the meaning of each explaining factor (latent coordinate) may
                  vary locally more than it needs to (i.e. the same CVAE architecture could do better
                  if a higher `latent_dim` was used).

                  Note that we have special visualization tools (see the `plotter` module) for
                  a 2D latent space, for working with small toy datasets and for debugging.

                  (But note that even MNIST digits cannot be compressed into just two independent factors.
                   Spectral embeddings indicate that the dimension of the MNIST digits manifold is
                   somewhere near 11.)
...
class CVAE(tf.keras.Model):
    ...
    The optimizer to be used for training should be configured as `model.compile(optimizer=...)`.

    This class defines its metrics and losses manually; those should **not** be configured in `compile`.

    Encoding takes pixel data `x` → parameters of variational posterior qϕ(z|x), namely `(μ, log σ)`
    for a multivariate, diagonal Gaussian. Reparameterization takes those parameters and yields a
    code point `z`. The code point is chosen stochastically, by drawing one sample from qϕ(z|x).
    ...
    which can be used as the pixel brightness as-is.
    ...
    If needed, the encoder and decoder `tf.keras.Model` instances can be separately accessed
    via the `encoder` and `decoder` attributes.
    """
    ...
    def __init__(self, *, latent_dim=2, variant=7):
        ...
        self.encoder, self.decoder = make_codec(variant, latent_dim)
