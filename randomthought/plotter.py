...
We include some special utilities for visualizing 2D latent spaces as a small-multiples plot,
with a quantile grid (useful when using the standard spherical normal latent prior).
"""
...
def plot_test_sample_image(test_sample: tf.Tensor, *,
    ...
    fig.tight_layout()
    plt.draw()
    plotutils.pause(0.1)  # force redraw
...
def plot_manifold(x: tf.Tensor,
    ...
    Dimension reduction in general is slow. We suggest limiting the amount of data::
        ...
        n = 4000
        plot_manifold(test_images[:n, :, :, :], test_labels[:n], model=model)
