...
The method is a variant of MLS, "moving least squares". The "moving" in the name is an anachronism; a classical
serial CPU implementation would solve the local systems one by one, thus explicitly "moving" the neighborhood.

This method produces approximations for the denoised function value, the jacobian, and the hessian in one go,
by solving local 6×6 linear equation systems.
...
The hifiest/recommended algorithm (`prepare`/`solve`) is also fully autodifferentiable, so that you can
differentiate the spatial jacobian and hessian with regard to e.g. neural network parameters (if a neural
network produced the input bitmap image), but how useful that is depends on exactly how your parameters
affect the pixels. This is an area that needs to be explored more fully.

We need only the very basics here, but we can take some VRAM and computation time saving shortcuts, because
we operate on a cartesian pixel grid. A complete Cython implementation of WLSQM on arbitrary point clouds,
