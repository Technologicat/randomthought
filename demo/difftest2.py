...
from randomthought.differentiate import prepare, solve_lu as solve, coeffs_full
# from randomthought import smoothing
...
def main():
    ...
    idx_to_name = list(coeffs_full.keys())  # list({v: k for k, v in coeffs_full.items()}.values())
    fig, axs = plt.subplots(2, 3, figsize=(12, 8))
    for j in range(6):
        row, col = divmod(j, 3)
        ax = axs[row, col]
        theplot = ax.pcolormesh(X, Y, dZ[j].numpy())
    ...
    #     row, col = divmod(j, 3)
    #     ax = axs[row, col]
    #     plot_one(ax, X, Y, dZ[j].numpy(), title=idx_to_name[j])
